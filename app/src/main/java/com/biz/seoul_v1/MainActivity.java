...
import com.biz.seoul_v1.helper.HostViewAdapter;
...
public class MainActivity extends AppCompatActivity {
    ...
    class Seoul_Delegate extends AsyncTask{
        ...
        // doInBackground에서 호출한 method가 모두 일으 ㄹ마치면
        // 최종 마지막 보고나, 화면정리 등을 하는 method가 있다.
        // AsyncTask에 의해서 어떤 method가 호출되고,
        // 모든 실행이 끝나면 view나 layout 등에 내용을 표시하는 일들을 하는 곳곳
       @Override
    ...
    void seoul_pet() {
        ...
        api_url += "/json/" ; // 받을 데이터 형식
        api_url += service_url ; // 서비스문자열

        api_url += "/1/100/" ; // 데이터를 100개 불러와라라
        ...
        try {
            ...
            if(resCode == 200) {
                ...
                while(true){
                    ...
                    // 임시 보관하는 방식인데,
                    // 안드로이드에서는 문자열 = 문자열 + 다른 문자열 방식은
                    // 성능상 약간의 문제가 있다고 하여
                    // StringBuffer를를 만들고 sb.append(다른문자) 형식으로 사용하라고 권장함.
                    sb.append(reader) ;
                    st += reader ;
                ...
                // jsonData로부터 필요한 부분만 잘라내기
                /*
                   서울시 공공 data로부터 return 받은 데이터의 구조
                   jsonData로부터
                   1. service_url 부분을 자르고
                   2. row부분을 잘라내어야 실제 원하는 데이터를 얻을 수 있다.
                 */
                ...
                // RecyclerView에 데이터 뿌리기를 하고 싶지만
                // AsyncTask 대리자가 실행하는 method에서는
                // 절대!!!! view, layout 등에 직접 데이터를 표현하는
                // 코드를 쓸 수 없다.

           } else{

                Log.i("서울시", "데이터 응답 오류 발생!!") ;
            ...
            // e.printStackTrace();
            Log.d("서울시", "문자열을 JSON으로 변환시키는데 오류 발생") ;
        }
    }
